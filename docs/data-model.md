...
| имя (Name) | отображаемое имя в интерфейсе (профиль) | строка, nullable |
...
| статус | согласованный набор состояний | перечисление (см. **API v1** ниже) |
...
**Значения статуса в HTTP API v1** (поле `status` в JSON, snake_case): `not_started`, `in_progress`, `completed`, `skipped`. Детали запросов/ответов — [docs/api/openapi-v1.yaml](api/openapi-v1.yaml).
