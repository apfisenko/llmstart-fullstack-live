...
| Роль | Назначение |
|------|------------|
| **Студент** | Участник потока: вопросы по курсу, навигация, свой прогресс и результаты. |
| **Преподаватель** | Ведение и обзор потока: статус группы, активность, сопровождение без дублирования каналов вручную. |
...
- у преподавателя — **просмотр активности группы** и опора для **сопровождения потока**.
...
flowchart TB
  ...
  S --> P["Прогресс и фиксация результата"]
  T --> G["Обзор группы и потока"]
  Q --> BE["Backend"]
  P --> BE
  G --> BE
```
...
Таблица отражает текущий и близкий горизонт без фиксации версий web-стека.
...
| LLM | OpenAI-совместимый клиент → OpenRouter (или аналог) | Вызовы только из backend по целевой архитектуре. |
| Качество Python | `ruff`, `make` | Линт, формат, задачи. |
| Конфиг | переменные окружения, валидация при старте | Fail fast при ошибках конфигурации. |
