...
| *(отдельных задач нет)* | Итерация 5 — интеграция клиентов: веб на **`/api/v1/`**; итерация **5** в [`plan.md`](../plan.md) — **✅ Done** (сквозной DoD, в т.ч. «бот → данные в вебе»); отдельных пунктов 12+ в этом tasklist не ведётся |
