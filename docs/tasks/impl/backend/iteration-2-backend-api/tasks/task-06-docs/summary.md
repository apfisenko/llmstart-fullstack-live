...
- На момент задачи обновлялись шаблоны env (в текущем репозитории канон — только [`backend/.env.example`](../../../../../../../backend/.env.example)): загрузка `.env`, формулировка про пустой `OPENROUTER_API_KEY`, `DATABASE_URL` и PostgreSQL.
